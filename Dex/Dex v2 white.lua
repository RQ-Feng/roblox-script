...
function CreateRightClickMenuItem(text, onClick, insObj)
	...
	button.MouseEnter:connect(function()
		...
		if not insObj and CurrentInsertObjectWindow then
			if CurrentInsertObjectWindow.Visible == false and button.Text == "插入对象" then
				CreateInsertObject()
			elseif CurrentInsertObjectWindow.Visible and button.Text ~= "插入对象" then
				CurrentInsertObjectWindow.Visible = false
			end
		end
		if not insObj then
...
function filteringWorkspace()
	if explorerFilter.Text ~= "" and explorerFilter.Text ~= "搜索Workspace..." then
		return true
	end
	return false
end
...
function rightClickMenu(sObj)
	...
	currentRightClickMenu = CreateRightClickMenu(
		{"剪切","复制","粘贴","克隆","删除","Group","Ungroup","选择所有子级","传送到...","插入部件","插入对象...","预览脚本","Save Instance","Call Function","Call Remote"},
		"",
		false,
		function(option)
			if option == "剪切" then
...
function searchingProperties()
	if propertiesSearch.Text ~= "" and propertiesSearch.Text ~= "搜索属性..." then
		return true
	end
	return false
end
